...
define("antie/widgets/grid", [
  ...
  var interval;
  var timeout;
  var Grid = Container.extend(
    /** @lends antie.widgets.Grid.prototype */ {
      /**
       * @constructor
       * @ignore
       */
      ...
      render: function render(device) {
        if (!this.outputElement) {
          this.outputElement = device.createContainer(
            this.id,
            this.getClasses()
          );
        } else {
        ...
        for (var row = 0; row < this._rows; row++) {
          ...
          for (var col = 0; col < this._cols; col++) {
            ...
            if (widget) {
              ...
              device.appendChildElement(
                rowElement,
                this.getWidgetAt(col, row).render(device)
              );
            } else {
              var classes = ["spacer"];
              if (col === 0) {
              ...
              device.appendChildElement(
                rowElement,
                device.createContainer(this.id + "_" + col + "_" + row, classes)
              );
            }
          }
      ...
      setActiveChildWidget: function setActiveChildWidget(widget) {
        ...
        if (setActiveChildWidget.base.call(this, widget)) {
          ...
          if (changed) {
            this.bubbleEvent(
              new SelectedItemChangeEvent(this, widget, selectedIndex)
            );
          }
          return true;
        } else {
      ...
      _onKeyDown: function _onKeyDown(evt, isNormalPress, self) {
        ...
        do {
          ...
          var _newSelectedIndex =
            _newSelectedRow * self._cols + _newSelectedCol;
          var _widget = self._childWidgetOrder[_newSelectedIndex];
          /* Select the next available column if there is no cells down*/
          if (!_widget) {
            for (var i = _newSelectedCol - 1; i >= 0; i--) {
              _newSelectedIndex = _newSelectedRow * self._cols + i;
              _widget = self._childWidgetOrder[_newSelectedIndex];
              if (_widget) {
                break;
              }
            }
