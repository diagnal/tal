...
define("antie/devices/device", [
    ...
    var Device = Class.extend(
        /** @lends antie.devices.Device.prototype */ {
            /**
             * @constructor
             * @ignore
             */
            init: function init(config) {
                ...
                this.filteredLoggingMethods = filterLoggingMethods(
                    config,
                    selectLoggingStrategy(config, this.loggingStrategies)
                );
                ...
                function selectLoggingStrategy(config, loggingStrategies) {
                    if (config.logging && config.logging.strategy) {
                        var configuredLoggingStrategy =
                            "antie/" +
                            "devices/" +
                            "logging/" +
                            config.logging.strategy;
                    ...
                    var selectedLoggingStrategy =
                        loggingStrategies["antie/devices/logging/default"];

                    if (!selectedLoggingStrategy) {
                        selectedLoggingStrategy =
                            loggingStrategies["antie/devices/logging/onscreen"];
                    }
            ...
            getScreenSize: function getScreenSize() {
                throw new Error(
                    "Device::getScreenSize not implemented by current Device subclass."
                );
            },
            /**
            ...
            crossDomainPost: function crossDomainPost(url, data, opts) {
                ...
                function iframeLoadedCallback() {
                    var urlLoadedIntoInvisibleIFrame,
                        errorGettingIFrameLocation;
                    try {
                        urlLoadedIntoInvisibleIFrame =
             ...
             */
            executeCrossDomainGet: function executeCrossDomainGet(
                url,
                opts,
                jsonpOptions
            ) {
             ...
             * in the request.
             */
            executeCrossDomainPut: function executeCrossDomainPut(
                url,
                data,
                opts
            ) {
             ...
             * in the request.
             */
            executeCrossDomainPost: function executeCrossDomainPost(
                url,
                data,
                opts
            ) {
            ...
            /**
             * Performs a cross domain DELETE HTTP using CORS
             * @param {String} url The URL to post to.
             * @param {Object} data JavaScript object to be JSON encoded and delivered as payload.
             * @param {Object} opts Object containing callback functions, a form field name and an optional bearer token.
             * @param {String} opts.fieldName Name to be used for the POST form field for form based (non-CORS) requests.
             ...
             * in the request.
             */
            executeCrossDomainDelete: function executeCrossDomainDelete(
                url,
                data,
                opts
            ) {
            ...
            getStorage: function getStorage(storageType, namespace, opts) {
                if (storageType === StorageProvider.STORAGE_TYPE_SESSION) {
                    ...
                    storageType === StorageProvider.STORAGE_TYPE_PERSISTENT
                ) {
    ...
    Device.load = function(config, callbacks) {
        try {
            require([config.modules.base].concat(
                config.modules.modifiers
