    ...
    /** @lends antie.devices.Device.prototype */ {
                ...
                case "alpha":
                  ...
                  for (kc = A; kc <= Z; kc++) {
                    this._keyMap[kc.toString()] =
                      symbolMap[String.fromCharCode(kc - A + AcharCode)];
                  }
                  break;
                case "numeric":
                  ...
                  for (kc = zero; kc <= nine; kc++) {
                    this._keyMap[kc.toString()] =
                      symbolMap[String.fromCharCode(kc - zero + zeroCharCode)];
                  }
                  break;
                default:
        ...
        function selectLoggingStrategy(config, loggingStrategies) {
          if (config.logging && config.logging.strategy) {
            var configuredLoggingStrategy =
              "antie/" + "devices/" + "logging/" + config.logging.strategy;
      ...
      loadScript: function loadScript(
        ...
        var funcName =
          "_antie_callback_" +
          (callbackSuffix ||
            new Date() * 1 + "_" + Math.floor(Math.random() * 10000000));
